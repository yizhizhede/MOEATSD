...
static void 
Householders_Reduction_to_Bidiagonal_Form(double* A, int nrows,
    int ncols, double* U, double* V, double* diagonal, double* superdiagonal );
static int  
Givens_Reduction_to_Diagonal_Form( int nrows, int ncols,
...
//                                                                            //
static int 
Singular_Value_Decomposition(double* A, int nrows, int ncols, double* U, 
                      double* singular_values, double* V, double* dummy_array)
{
	if (nrows < ncols) {
		fprintf(stderr,"%s:%d nrows should >= ncols, but nrow: %d < ncol %d \n",
			__FILE__,__LINE__,nrows,ncols);
		return -1;
	};
...
static void 
Singular_Value_Decomposition_Solve(double* U, double* D, double* V,  
                double tolerance, int nrows, int ncols, double *B, double* x) 
{
   	int i,j,k;
   	double *pu, *pv;
...
Tra(double *A,int nrows,int ncols,double *B)
{
         ...
         for (i=0,pb=B; i<ncols; i++) {
                 for (j=0,pa=A; j<nrows; j++,pb++,pa+=ncols) {
                         *pb = *(pa+i);
                 }
         }
}
...
svd(double *A,int nrows,int ncols,double *U,double *D,double *V)
{
	...
	dummy_array = (double *)malloc((nrows+ncols)*sizeof(double));
	
	if (dummy_array) {
		if (nrows >= ncols) {
			Singular_Value_Decomposition(A,nrows,ncols,U,D,V,dummy_array);
		} else {
			tA = (double *)malloc(nrows*ncols*sizeof(double));
			Tra(A,nrows,ncols,tA);
			Singular_Value_Decomposition(tA,ncols,nrows,V,D,U,dummy_array);
			free(tA);
		}
	}
...
//	int i;
	...
	U = (double *)malloc(nrows*nrows*sizeof(double));
	D = (double *)malloc((nrows+nrows)*sizeof(double));
	V = (double *)malloc(nrows*nrows*sizeof(double));

	// singular value decomposition
	svd(A,nrows,nrows,U,D,V);

	// detect whether or not reversible
	//for (i=0; i<nrows; i++)	if (D[i] < epsilon) {
	//	fprintf(stderr,"try to inverse a matrix witch is not reversible\n");
	//	exit(0);		
	//}
	
	Singular_Value_Decomposition_Inverse(U,D,V,0,nrows,nrows,B);	
...
Coo(double *A,int nrows,double *B,double *x)
{
	...
	Singular_Value_Decomposition_Solve(U,D,V,0.0,nrows,nrows,B,x);
