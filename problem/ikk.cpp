#include <string.h>
#include <stdio.h>
#include <stdlib.h>
#include <math.h>

#include "ikk.h"
#include "matrix.h"

#define PI 3.14159265358979323846264338327950288419716939937510
#define pi 3.14159265358979323846264338327950288419716939937510
#define NUM_SAMPLE 1.0e+4

// test problem
static Problem_t *P;

// 
static void  ikk1 (double *x, int n, double *f, int m);

Problem_t* IKK_new (char *title, int numObj, int numVar) {
	// common variable
	int 	i;
	size_t 	size;
	double*	lowBound = NULL; 
	double* uppBound = NULL;

	// allocating memory for a problem
        Problem_t *problem = (Problem_t *)malloc (sizeof (Problem_t));
        if (problem == NULL) {
        	fprintf (stderr, "Allocating memory failed\n");         
               	exit (-1);
        }
	strcpy (problem->title, title);
	problem->numObj = 3; 
	problem->numVar = 2;
	
	// setting the bound of varibles
	size = problem->numVar * sizeof (double);
	lowBound = (double *)malloc (size);
	uppBound = (double *)malloc (size);
	for (i=0; i<numVar; i++) {
		lowBound[i] = -50.0;
		uppBound[i] = 50.0;
	}
	problem->lowBound = lowBound;
	problem->uppBound = uppBound;

	// 
	if (!strcmp (title, "IKK1")) {
		P = problem; problem->evaluate = ikk1;
	} else { 
		fprintf (stderr, "error: %s is undefined\n", title);
		exit (0);
	}

	return problem;
}

static void  ikk1 (double *x, int n, double *f, int m) {
	f[0] = x[0]*x[0];
	f[1] = (x[0]-20)*(x[0]-20);
	f[2] = x[1]*x[1];
}


//**************************************************************************************************************
//**************************************************************************************************************
static Matrix_t* IKK1_sample ();

Matrix_t* IKK_sample (int No, int numObj) {
	switch (No) {
		case 1:
			return	IKK1_sample ();
		default:
                       fprintf (stderr, "IKK%d_sample have been not implemented now\n", No);
                       exit (0);
	}
	return NULL;
}

static Matrix_t* IKK1_sample () {
	int 		i;
	double 		d = 20.0/NUM_SAMPLE;
	double		x[10];
	Matrix_t*	sample = Matrix_new (NUM_SAMPLE+1, 3);

	for (i=0; i<=NUM_SAMPLE; i++) {
		x[0] = i*d;
		x[1] = 0;
		ikk1 (x, 2, sample->elements+3*i, 3);
	}
	
	return sample;
}
