#include <string.h>
#include <stdio.h>
#include <stdlib.h>
#include <math.h>

#include "jos.h"
#include "matrix.h"
#include "shape.h"

#define PI 3.14159265358979323846264338327950288419716939937510
#define pi 3.14159265358979323846264338327950288419716939937510
#define NUM_SAMPLE 1.0e+4

// test problem
static Problem_t *P;

// 
static void  jos1 (double *x, int n, double *f, int m);
static void  jos2 (double *x, int n, double *f, int m);

Problem_t* JOS_new (char *title, int numObj, int numVar) {
	// common variable
	int 	i;
	size_t 	size;
	double*	lowBound = NULL; 
	double* uppBound = NULL;

	// allocating memory for a problem
        Problem_t *problem = (Problem_t *)malloc (sizeof (Problem_t));
        if (problem == NULL) {
        	fprintf (stderr, "Allocating memory failed\n");         
               	exit (-1);
        }
	strcpy (problem->title, title);
	problem->numObj = 2; 
	problem->numVar = numVar;
	
	// setting the bound of varibles
	size = numVar * sizeof (double);
	lowBound = (double *)malloc (size);
	uppBound = (double *)malloc (size);
	for (i=0; i<numVar; i++) {
		lowBound[i] = 0.0;
		uppBound[i] = 1.0;
	}
	if (!strcmp (title, "JOS1")) {
		for (i=0; i<numVar; i++) {
			lowBound[i] = -100;
			uppBound[i] = 100;
		}
	}
	problem->lowBound = lowBound;
	problem->uppBound = uppBound;

	// 
	if (!strcmp (title, "JOS1")) {
		P = problem; problem->evaluate = jos1;
	} else if (!strcmp (title, "JOS2")) {
		P = problem; problem->evaluate = jos2;
	} else { 
		fprintf (stderr, "error: %s is undefined\n", title);
		exit (0);
	}

	return problem;
}

static void  jos1 (double *x, int n, double *f, int m) {
	int 	i;
	double 	g = 0;

	// f1
	for (i=0, g=0; i<n; i++) {
		g += x[i]*x[i];
	}
	g = g/n;
	f[0] = g;

	// f2
	for (i=0, g=0; i<n; i++) {
		g += (x[i]-2)*(x[i]-2);
	}
	g = g/n;
	f[1] = g;
}

static void  jos2 (double *x, int n, double *f, int m) {
	int 	i;
	double 	g = 0, h;

	// f1
	f[0] = x[0];
	
	// g
	for (i=1, g=0; i<n; i++) {
		g += x[i];
	}
	g = 1 + 9.0*g/(n-1);
	
	// h
	h = 1 - pow(1-f[0]/g, 0.25) - pow(f[0]/g, 4.0);

	// f2
	f[1] = g*h;
}


//**************************************************************************************************************
//**************************************************************************************************************
static Matrix_t* JOS1_sample ();
static Matrix_t* JOS2_sample ();

Matrix_t* JOS_sample (int No, int numObj) {
	switch (No) {
		case 1:
			return	JOS1_sample ();
		case 2:
			return	JOS2_sample ();
		default:
                       fprintf (stderr, "JOS%d have been not implemented now\n", No);
                       exit (0);
	}
	return NULL;
}

static Matrix_t* JOS1_sample () {
	int 		i;
	double 		d = 2.0/NUM_SAMPLE;
	double  	x[10];
	Matrix_t*	sample = Matrix_new (NUM_SAMPLE+1, 2);

	for (i=0; i<=NUM_SAMPLE; i++) {
		x[0] = i * d;
		jos1(x, 1, sample->elements+2*i, 2);
	}
	return sample;
}

static Matrix_t* JOS2_sample () {
	int 		i;
	double 		d = 1.0/NUM_SAMPLE;
	double		x[10];
	Matrix_t*	sample = Matrix_new (NUM_SAMPLE+1, 2);

	for (i=0; i<=NUM_SAMPLE; i++) {
		x[0] = i * d; 
		x[1] = 0;
		jos2(x, 2, sample->elements+2*i, 2);
	}
	
	return sample;
}
