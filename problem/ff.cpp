...
// 
static void  ff1 (double *x, int n, double *f, int m);
...
Problem_t* FF_new (char *title, int numObj, int numVar) {
	...
	// 
	if (!strcmp (title, "FF1")) {
		P = problem; problem->evaluate = ff1;
	} else { 
...
static void  ff1 (double *x, int n, double *f, int m) {
	f[0] = 1.0 - exp (-(x[0]-1.0)*(x[0]-1.0)-(x[1]+1.0)*(x[1]+1.0));
	f[1] = 1.0 - exp (-(x[0]+1.0)*(x[0]+1.0)-(x[1]-1.0)*(x[1]-1.0));
}


//**************************************************************************************************************
//**************************************************************************************************************
static Matrix_t* FF1_sample ();
...
Matrix_t* FF_sample (int No, int numObj) {
	switch (No) {
		case 1:
			return	FF1_sample ();
		default:
                       fprintf (stderr, "FF%d_sample have been not implemented now\n", No);
                       exit (0);
	}
	return NULL;
}
