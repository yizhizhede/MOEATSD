...
// 
static void  im1 (double *x, int n, double *f, int m);
...
Problem_t* IM_new (char *title, int numObj, int numVar) {
	...
	// 
	if (!strcmp (title, "IM1")) {
		P = problem; problem->evaluate = im1;
	} else { 
...
Matrix_t* IM_sample (int No, int numObj) {
	switch (No) {
		case 1:
			return	IM1_sample ();
		default:
                       fprintf (stderr, "IM%d_sample have been not implemented now\n", No);
                       exit (0);
	}
	return NULL;
}
