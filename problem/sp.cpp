...
// 
static void  sp1 (double *x, int n, double *f, int m);
...
Problem_t* SP_new (char *title, int numObj, int numVar) {
	...
	// 
	if (!strcmp (title, "SP1")) {
		P = problem; problem->evaluate = sp1;
	} else { 
...
static void  sp1 (double *x, int n, double *f, int m) {
	f[0] = (x[0]-1)*(x[0]-1) + (x[0]-x[1])*(x[0]-x[1]);
	f[1] = (x[1]-3)*(x[1]-3) + (x[0]-x[1])*(x[0]-x[1]);
}

//**************************************************************************************************************
//**************************************************************************************************************
static Matrix_t* SP1_sample ();
...
Matrix_t* SP_sample (int No, int numObj) {
	switch (No) {
		case 1:
			return	SP1_sample ();
		default:
                       fprintf (stderr, "SP%d have been not implemented now\n", No);
                       exit (0);
	}
	return NULL;
}
